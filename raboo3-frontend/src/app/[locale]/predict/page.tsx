...
export default function PredictPage() {
  ...
  const handleGetBestArea = useCallback(async () => {
    ...
    try {
      ...
      const propertyTypeAr = usage === 'سكني' ? 'قطعة أرض-سكنى' : 'قطعة أرض-تجارى'
              ...
              <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
                <p className="text-sm font-medium text-ink-900 dark:text-white">
                  ...
                  {result.district && result.district !== result.city
                    ? translateDistrict(result.district, locale)
                    : translateCity(result.city, locale)}
                  {result.district && result.district !== result.city
                    ? (isAr ? '، ' : ', ') + translateCity(result.city, locale)
                    : ''}
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="card p-0 overflow-hidden relative">
            ...
            <GoogleMapView
              ...
              top3Areas={result?.bestAreas?.map((a) => ({
                lat: a.latitude,
                lng: a.longitude,
                label: (a.district ? translateDistrict(a.district, locale) : translateCity(a.city, locale)),
              })) ?? null}
