...
export default function InsightsPage() {
    ...
    fetch('/api/insights')
      ...
      .catch((e) => {
        ...
        setError(
          isFetchFailed
            ? (isAr
                ? 'تعذر الاتصال بالخادم. إمّا اضبطي NEXT_PUBLIC_SUPABASE_URL وSUPABASE_SERVICE_ROLE_KEY مع بيانات التجميع في Supabase، أو شغّلي الباك اند (raboo3-ml: uvicorn api.main:app --reload --port 8000) مع ML_API_URL في .env.local.'
                : 'Could not reach the server. Either set NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY with aggregate data in Supabase, or run the ML backend (uvicorn api.main:app) and set ML_API_URL in .env.local.')
  ...
  const chartData = CITIES.map((city) => {
    ...
    return {
      city: translateCity(city, locale),
      ...
      <header className="space-y-3">
        ...
        {meta.year != null && meta.quarter != null && (
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {isAr ? `البيانات لأحدث سنة متاحة: ${meta.year}` : `Data for latest available year: ${meta.year}`}
          </p>
        )}
      </header>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {CITIES.map((city) => {
          ...
          return (
            ...
            >
              <h3 className="text-lg font-bold text-ink-900 dark:text-white">{translateCity(city, locale)}</h3>
