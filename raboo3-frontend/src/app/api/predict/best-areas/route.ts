...
export async function POST(req: Request) {
  try {
    ...
    if (typeof min_price_per_sqm === 'number' && min_price_per_sqm >= 0) body.min_price_per_sqm = min_price_per_sqm
    if (typeof max_price_per_sqm === 'number' && max_price_per_sqm >= 0) body.max_price_per_sqm = max_price_per_sqm
    ...
    try {
      res = await fetch(`${baseUrl}/recommend/districts`, {
        method: 'POST',
      ...
      return NextResponse.json(
        {
          error:
            'خدمة التقدير غير متاحة. شغّلي الباك اند أولاً: من مجلد raboo3-ml شغّلي: uvicorn api.main:app --reload --port 8000',
          detail: msg,
        },
        { status: 503 }
      )
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: message }, { status: res.status === 404 ? 503 : res.status })
    }
