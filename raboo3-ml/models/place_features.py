...
المصدر: Google Places.
- المرافق: google_places_services.csv (من Google Places).
- مراكز الأحياء: district_centroids.json (من Google Geocoding)، أو قيم ثابتة للشرقية فقط عند غياب الملف.
"""
...
def _load_pois_by_type() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """تحميل مرافق من Google Places (google_places_services.csv)."""
...
def _load_city_centroids() -> dict[str, tuple[float, float]]:
    """مركز كل مدينة: district_centroids.json (متوسط إحداثيات أحياء المدينة). قيم ثابتة للشرقية فقط عند غياب الملف."""
...
def build_place_features_table(
