...
class BestAreaItem(BaseModel):
    ...
    district_id: Optional[int] = Field(None, description="معرّف الحي في جدول District (Supabase)")
    property_type_ar: Optional[str] = Field(None, description="نوع العقار المرتبط بالمفضلة/التقدير")
...
class RecommendDistrictsRequest(BaseModel):
    """طلب اقتراح أفضل أحياء داخل نطاق (دائرة أو مستطيل bbox)."""
    ...
    bbox: Optional[list[float]] = Field(
        ...
        min_length=4,
        max_length=4,
        description="مستطيل جغرافي [min_lng, min_lat, max_lng, max_lat]. إن وُجد تُعاد فقط الأحياء داخل المستطيل.",
    )
...
class RecommendDistrictItem(BaseModel):
    ...
    city_ar: str
    district_ar: str
    district_id: Optional[int] = Field(None, description="معرّف الحي في جدول District")
    lat: float
    lon: float
