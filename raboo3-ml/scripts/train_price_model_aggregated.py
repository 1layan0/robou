"""Train aggregated price model (district-quarter level).

- Tries min_deals in [2, 3, 4, 5, 7, 10], picks best by R², saves only that model.
- Input: data/features/district_quarter_md{md}.csv (from build_district_quarter_dataset.py)
- Target: y = target_log (or target_resid if PREDICT_TARGET_LOG=False); evaluation on target_median_price_per_sqm
...
# التنبؤ بـ target_log مباشرة غالباً يعطي R² أعلى من residual
PREDICT_TARGET_LOG = True
...
def _prepare_df(df: pd.DataFrame, target_col: str | None = None) -> pd.DataFrame:
    """Fill numeric NaNs and ensure feature columns exist."""
    target_col = target_col or (TARGET_LOG_COL if PREDICT_TARGET_LOG else TARGET_RESID_COL)
    df = df.dropna(subset=[target_col, TARGET_PRICE_COL, BASELINE_LOG_COL, "deals_count"])
    ...
    if "growth_pct" in df.columns:
        df["growth_pct"] = pd.to_numeric(df["growth_pct"], errors="coerce").fillna(0.0)
    for c in ["quarter_sin", "quarter_cos"]:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce")
        if c in df.columns and df[c].isna().any():
            df[c] = df[c].fillna(0.0)
    if "year_quarter_idx" in df.columns:
        df["year_quarter_idx"] = pd.to_numeric(df["year_quarter_idx"], errors="coerce")
        df["year_quarter_idx"] = df["year_quarter_idx"].fillna(df["year"].astype(int) * 4 + df["quarter"].astype(int))
    return df


def train_and_evaluate_one(md: int) -> dict:
    """Load district_quarter_md{md}.csv, train model (residual or target_log), return model + metrics."""
    ...
    required = [TARGET_PRICE_COL, BASELINE_PRICE_COL, BASELINE_LOG_COL, "deals_count"]
    if not PREDICT_TARGET_LOG:
        required.append(TARGET_RESID_COL)
    else:
        required.append(TARGET_LOG_COL)
    for c in required:
        if c not in df.columns:
            raise ValueError(f"Missing column {c} in {path.name}")
    target_col = TARGET_LOG_COL if PREDICT_TARGET_LOG else TARGET_RESID_COL
    df = _prepare_df(df, target_col=target_col)

    feature_cols = [c for c in CAT_COLS + NUM_COLS if c in df.columns]
    X = df[feature_cols].copy()
    y_train_vals_full = df[target_col].astype(float).values
    ...
    y_train_vals = df.loc[train_idx, target_col].astype(float).values
    y_eval_vals = df.loc[test_idx, target_col].astype(float).values
    ...
    try:
        ...
        model = cb.CatBoostRegressor(
            iterations=2000, depth=9, learning_rate=0.03, l2_leaf_reg=2,
            random_seed=RANDOM_STATE, verbose=0, cat_features=cat_indices,
        )
        model.fit(
            X_train, y_train_vals,
            eval_set=(X_test, y_eval_vals),
        ...
        model.fit(X_train, y_train_vals, eval_set=[(X_test, y_eval_vals)])
        model_name = "LGBMRegressor"

    if PREDICT_TARGET_LOG:
