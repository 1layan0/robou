...
# أحدث (year, quarter) متاح لكل (city, district, prop) لتحسين تغطية الأسعار
_dq_latest_by_district: dict[tuple[str, str, str], dict[str, Any]] = {}
...
def _load_dq_cache() -> None:
    ...
    global _dq_map, _dq_latest_by_district, _dq_city_type_map, _dq_global_map, _dq_loaded
    if _dq_loaded:
        return
    ...
    for _, row in df.iterrows():
        ...
        # أحدث فترة لكل (مدينة، حي، نوع): نحمّل بالأقدم أولاً ثم نحدّث؛ نرتب لاحقاً
        key_cdt = (c, d, t)
        if key_cdt not in _dq_latest_by_district or (y, q) > (
            _dq_latest_by_district[key_cdt].get("year", 0),
            _dq_latest_by_district[key_cdt].get("quarter", 0),
        ):
            _dq_latest_by_district[key_cdt] = {"year": y, "quarter": q, **stats}
...
def _get_place_features(city_district_pairs: list[tuple[str, str]]) -> dict[tuple[str, str], dict[str, float]]:
    """استدعاء build_place_features_table وإرجاع قاموس (city, district) -> {dist_*, count_*}."""
    out = {}
    try:
        from models.place_features import build_place_features_table
        df = build_place_features_table(city_district_pairs)
        if df.empty:
...
def predict_price_for_districts(
    ...
    place_lookup = _get_place_features(pairs)
    ...
    for d in districts:
        ...
        place = place_lookup.get((c_ar, d_ar), {})
        key_dq = (c_ar, d_ar, prop, year, quarter)
        key_ct = (c_ar, prop, year, quarter)
        key_cdt = (c_ar, d_ar, prop)
        if key_dq in _dq_map:
            stats = _dq_map[key_dq]
            price_source = "model_district_q"
            use_year, use_quarter = year, quarter
        elif key_cdt in _dq_latest_by_district:
            stats = _dq_latest_by_district[key_cdt]
            use_year = int(stats.get("year", year))
            use_quarter = int(stats.get("quarter", quarter))
            price_source = "model_district_q"
        ...
        q = int(use_quarter) if use_quarter is not None else 1
        growth_info = growth_lookup.get((c_ar, d_ar, prop), {})
        growth_pct = float(growth_info.get("growth_pct", 0.0))
        row = {
            "city_ar": c_ar,
            "district_ar": d_ar,
            ...
            "latitude": lat,
            "longitude": lon,
            "dist_school_km": place.get("dist_school_km", 99.0),
            "dist_hospital_km": place.get("dist_hospital_km", 99.0),
            "dist_mall_km": place.get("dist_mall_km", 99.0),
            "count_school_3km": place.get("count_school_3km", 0),
            "count_hospital_3km": place.get("count_hospital_3km", 0),
            "count_mall_3km": place.get("count_mall_3km", 0),
            "growth_pct": growth_pct,
            "quarter_sin": np.sin(2.0 * np.pi * (q - 1) / 4.0),
            "quarter_cos": np.cos(2.0 * np.pi * (q - 1) / 4.0),
            "year_quarter_idx": int(use_year) * 4 + q if use_year is not None else 0,
        }
        rows.append(row)
    ...
    try:
        ...
        target_col = meta.get("target_col", "target_resid")
        if target_col == "target_log":
            pred_log = pred
        else:
            baseline_log = np.log1p(X["baseline_price_per_sqm"].values)
            pred_log = baseline_log + pred
...
# (city_ar, property_type_ar) -> growth_pct للنمو على مستوى المدينة (لبطاقة التحليلات)
_city_growth_lookup: dict[tuple[str, str], float] | None = None

# نوع العقار الافتراضي لصفحة التحليلات (سكني)
INSIGHTS_DEFAULT_PROPERTY_TYPE = "قطعة أرض-سكنى"

# حد أقصى لعرض النمو في بطاقة المدينة (تجنب نسب غير واقعية من عيّنات صغيرة أو بيانات شاذة)
MAX_DISPLAY_GROWTH_PCT = 30.0
MIN_DISPLAY_GROWTH_PCT = -20.0


def _load_city_growth_yoy() -> dict[tuple[str, str], float]:
    """تحميل city_growth_yoy.csv مرة واحدة. يُستخدم لعرض نمو المدينة ككل في بطاقة التحليلات."""
    global _city_growth_lookup
    if _city_growth_lookup is not None:
        return _city_growth_lookup
    _city_growth_lookup = {}
    path = PROJECT_ROOT / "data" / "features" / "city_growth_yoy.csv"
    if path.exists():
        df = pd.read_csv(path, encoding="utf-8-sig")
        # تطبيع أسماء الأعمدة (إزالة BOM إن وُجد)
        ...
        for _, row in df.iterrows():
            ...
            if not c or not t:
                continue
            pct = row.get("yoy_growth_3y_avg_pct")
            if pd.isna(pct):
                pct = row.get("yoy_growth_last_year_pct")
            if pd.isna(pct):
                pct = 0.0
            _city_growth_lookup[(c, t)] = float(pct)
    return _city_growth_lookup
...
def get_insights_data(
    ...
    # إحصائيات المدن: متوسط سعر، مجموع معاملات، نمو على مستوى المدينة (إن وُجد)، متوسط طلب
    city_growth = _load_city_growth_yoy()
    ...
    for c in ALLOWED_CITIES:
        ...
        if not subset:
            ...
            continue
        # نمو المدينة: من city_growth_yoy (انعكاس حقيقي للمدينة ككل)، وإلا أي صف للمدينة، وإلا متوسط أحياء
        city_level_growth = city_growth.get((c, prop))
        if city_level_growth is None and city_growth:
            for (cc, tt), pct in city_growth.items():
                if cc == c:
                    city_level_growth = pct
                    break
        if city_level_growth is not None:
            avg_growth = round(float(city_level_growth), 1)
        else:
            avg_growth = round(sum(r["growth"] for r in subset) / len(subset), 1)
        # تحديد النسبة المعروضة ضمن نطاق معقول (تجنب 80%+ من عيّنات صغيرة أو شذوذ)
        avg_growth = max(MIN_DISPLAY_GROWTH_PCT, min(MAX_DISPLAY_GROWTH_PCT, avg_growth))
        city_stats[c] = {
